...
`;
  ...
  try {
    ...
    console.warn("⚠️ Eroare OpenAI/JSON invalid:", err.message);
    return {
      meta_title: title,
...
async function generateBlogArticle() {
  const prompt = `
Scrie un articol SEO pentru blogul Sofipex.ro despre ambalaje, cutii pizza, caserole, pahare și tacâmuri biodegradabile.
Include:
- titlu principal (H1)
- 2 subtitluri (H2)
- conținut HTML profesional, fără coduri bloc sau backticks
- meta title (max 60 caractere)
- meta descriere (max 160 caractere)
- 3 taguri SEO
Răspunde doar cu HTML curat, fără delimitatori.
`;
...
async function fetchGSCData() {
  try {
    ...
    return rows.map((r) =>
      `• ${r.keys[0]} — ${r.clicks} clickuri, ${r.impressions} afișări, CTR ${(r.ctr * 100).toFixed(1)}%`
    ).join("<br>");
  } catch (err) {
...
async function runSEOAutomation() {
  ...
  for (const product of products.slice(0, 10)) {
    ...
    raport += `<li>✅ ${product.title} – actualizat SEO</li>`;
  }

  const blogBody = await generateBlogArticle();
  const blogTitle = await postBlogArticle(blogBody);
  raport += `</ul><p>📰 Articol generat: <b>${blogTitle}</b> (draft)</p>`;

  const gscData = await fetchGSCData();
  raport += `<h3>🔍 Cuvinte cheie GSC (ultimele 7 zile):</h3><p>${gscData}</p>`;
...
/* === Rulează zilnic la ora 08:00 România === */
